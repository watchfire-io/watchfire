...
Phoenix rises from a `project.yaml` that got reduced to ashes. A non-atomic write race in `config.SaveYAML` let a concurrent reader see a truncated file mid-write; `yaml.Unmarshal` silently returned a zero-valued `Project` struct (no error), `SyncNextTaskNumber` then bumped one field on that zero struct and saved it back — clobbering `project_id`, `name`, `definition`, timestamps, and every other field while preserving only `next_task_number`. The same bug also wiped `~/.watchfire/projects.yaml`. Phoenix closes the race at the OS level, adds load-time validation, and hardens the one writer that historically caused the data loss. The release also lands two pieces of v6.0 scope that had been queued up: the flock-based singleton-daemon hardening that prevents double-spawn (the bug that let two `watchfired` processes own different gRPC ports and two menu-bar tray icons simultaneously), and Cursor Agent CLI as a sixth first-class agent backend.

### Added

- **Cursor Agent CLI as a sixth first-class agent backend (#0088, closes upstream issue #34).** New `internal/daemon/agent/backend/cursor.go` implementing the full `Backend` interface alongside the five existing backends (Claude Code, Codex, opencode, Gemini, Copilot). Mirrors the Copilot backend's structure: per-session `~/.watchfire/cursor-home/<project_id>/<session_id>/` directory with the user's real `~/.cursor/` auth/config files symlinked in (missing files skipped gracefully), composed Watchfire system prompt installed as `AGENTS.md`, headless `cursor-agent --workspace <worktree> --print` launch with the yolo / trust flag set, JSONL transcript located via `LocateTranscript` and rendered through `FormatTranscript` in the same `## Role\n\n...` shape every other backend uses. `internal/daemon/metrics/parser.go::GetParser` handles `cursor` / `cursor-agent`. `DefaultSettings.Agents` includes `"cursor": {Path: ""}`. TUI (`internal/tui/globalsettings.go`) and GUI (`Settings/AgentPathsSection.tsx`, `Settings/searchIndex.ts`, `AddProject/StepAgent.tsx`) include `cursor` in search keywords + the agent picker. Selection chain (per-task → per-project → global default → `claude-code` fallback) treats `"cursor"` as a valid value with no special-casing. 473 LOC of impl + 403 LOC of tests; registry is additive so existing tests are unaffected.

### Fixed

- **Atomic YAML writes — closes the project.yaml data-loss race.** `internal/config/loader.go::SaveYAML` now writes to a sibling tmp file (`os.CreateTemp(dir, base+".*.tmp")`), `fsync`s, then `os.Rename`s into place. POSIX rename is atomic on the same filesystem, so concurrent readers see either the old file or the new file — never a truncated/empty one. This single fix eliminates the race for *every* YAML file the daemon writes (`project.yaml`, the global `projects.yaml`, task files, settings, agents, daemon state, integrations).
- **`LoadProject` rejects zero-valued reads.** `internal/config/projects.go::LoadProject` now returns `corrupt project.yaml at <path>: version=… project_id=…` when the unmarshalled struct has `Version == 0` or empty `ProjectID`. Belt-and-braces: `yaml.Unmarshal` is too forgiving on empty content (no error, zero struct), and any future writer that introduces a similar race will surface as a load error instead of silently rolling forward with garbage.
- **`SyncNextTaskNumber` refuses to round-trip an incomplete project.** `internal/config/tasks.go::SyncNextTaskNumber` is the function that historically did the damage — load → mutate `NextTaskNumber` → save. It now bails with an explicit error if the loaded project's `Version == 0` or `ProjectID == ""`, so this code path can never again be the writer that clobbers good metadata, even if `LoadProject`'s validation is ever relaxed.
- **Double-daemon spawn — flock-based singleton hardening (#0087).** Reproduced 2026-05-05 with two `watchfired` processes running simultaneously, each owning a separate gRPC server on a different dynamic port and a separate macOS menu-bar tray icon; GUI clients silently connected to whichever server happened to bind first while `~/.watchfire/daemon.yaml` advertised the other. Root cause was a TOCTOU race in `runDaemon`: the legacy `IsDaemonRunning()` check read `daemon.yaml` + `kill -0`'d the PID, but `daemon.yaml` is only written at the end of `onStart` behind a 100ms–5s `waitForPort` poll. Two processes launching in that window both passed the check, both bound dynamic ports, both spawned tray icons, and both raced to write `daemon.yaml` (last write wins). New `internal/config/lock.go` + `lock_unix.go` (real `syscall.Flock(LOCK_EX|LOCK_NB)`) + `lock_windows.go` (no-op stub for cross-builds; bug is macOS-specific and Linux + macOS share the syscall path) expose `AcquireDaemonLock()` returning `ErrDaemonLockHeld` on contention. `internal/daemon/cmd/root.go::runDaemon` now acquires the lock on `~/.watchfire/daemon.lock` before any tray/server init and holds it for the full process lifetime. Duplicate spawn logs `daemon already running, exiting` and returns cleanly with status 0. The lockfile is never deleted on release — flock release is tied to file-handle close (which the OS guarantees on process exit including `SIGKILL`), and deletion would open its own race window. `daemon.yaml` semantics + CLI/GUI `EnsureDaemon` paths are unchanged.

### Migration

- All Phoenix changes are internal; no schema or API changes. Existing `project.yaml` files load unchanged.
- Test fixtures that constructed `&models.Project{...}` literals without setting `Version: 1` will now fail to load via `config.LoadProject`. Production code paths use `models.NewProject` (which sets `Version: 1`) and are unaffected. One in-tree fixture in `internal/daemon/task/manager_test.go` was updated as part of this release; downstream forks with similar fixtures will need the same one-line addition.
...
- Cursor backend: `cursor-agent` CLI must be on `PATH` (or its absolute path set in `~/.watchfire/agents.yaml::cursor.Path`). Existing projects continue to use whatever `default_agent` they had; opt in via `watchfire configure` or by editing `default_agent: cursor` in `.watchfire/project.yaml`.

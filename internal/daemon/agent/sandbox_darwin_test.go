...
// TestGenerateProfile_KeychainDirWritable pins the keychain write rule.
// It must cover the CONTAINING DIRECTORY: the keychain is replaced
// atomically via a temp file created alongside it, so allowing only the
// db files makes the write fail with EPERM — and then /login reports
// success while the refreshed token silently fails to persist, leaving
// the session in a 401 loop.
func TestGenerateProfile_KeychainDirWritable(t *testing.T) {
	policy := DefaultPolicy("/home/test", "/projects/foo", backend.SandboxExtras{})
	got := GenerateProfile(policy)

	want := `(allow file-write* (subpath "/home/test/Library/Keychains"))`
	if !strings.Contains(got, want) {
		t.Fatalf("profile missing keychain dir write allow:\nwant line: %s", want)
	}
	// The old file-literal rules cannot satisfy the atomic replace and
	// must not come back as the only allowance.
	if strings.Contains(got, `(allow file-write* (literal "/home/test/Library/Keychains/login.keychain-db"))`) &&
		!strings.Contains(got, want) {
		t.Error("keychain allowed only as file literals — atomic replace will EPERM")
	}
}
